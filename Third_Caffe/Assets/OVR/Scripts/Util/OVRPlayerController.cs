...
{	
	...
	protected virtual void Update()
	{
		...
		if (Input.GetKey (KeyCode.W)) {
			GameObject.Find ("up_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
		}
		if (Input.GetKey (KeyCode.A)) {
			GameObject.Find ("left_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
		}
		if (Input.GetKey (KeyCode.S)) {
			GameObject.Find ("down_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
		}
		if (Input.GetKey (KeyCode.D)) {
			GameObject.Find ("right_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
		}
		if (!Input.GetKey (KeyCode.W)) {
			GameObject.Find ("up_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
		}
		if (!Input.GetKey (KeyCode.A)) {
			GameObject.Find ("left_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
		}
		if (!Input.GetKey (KeyCode.S)) {
			GameObject.Find ("down_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
		}
		if (!Input.GetKey (KeyCode.D)) {
			GameObject.Find ("right_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
		}
		if (Input.GetKeyDown (KeyCode.Space)) {
			GameObject.Find ("pular_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
		}
		if (!Input.GetKey (KeyCode.Space)) {
			GameObject.Find ("pular_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
		}
		...
		if (Controller.isGrounded && FallSpeed <= 0)
			FallSpeed = ((Physics.gravity.y * (GravityModifier * 0.0007f)));
		else
			FallSpeed += ((Physics.gravity.y * (GravityModifier * 0.0007f)) * SimulationRate * Time.deltaTime);
	...
	public bool Jump()
	{
		...
		//check rotation
		Vector3 angle = GameObject.Find ("ForwardDirection").GetComponent<Transform> ().forward;

		MoveThrottle += new Vector3(0.9f * angle.x, JumpForce * 2, 0.9f * angle.z);
		return true;
	}
	...
	void OnTriggerEnter(Collider other) {
		if (other.gameObject.tag.Equals ("Coin")) {
			Debug.Log ("Colidiu");
			moedas++;
			GameObject.Find ("Text_moedas").GetComponent<Text> ().text = "Moedas Coletadas: " + moedas;

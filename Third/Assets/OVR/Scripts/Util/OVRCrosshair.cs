	...
	public void UpdateCrosshair()
	{
		if (ShouldDisplayCrosshair())
		{
			...
			if (Physics.Linecast (startPos, endPos, out hit)) {				
				...
				if (hit.collider.gameObject.tag == "Coin" && Input.GetKeyDown(KeyCode.K)) {					
					selectedObject = hit.collider.gameObject;
					selectedObject.GetComponent<MeshRenderer> ().material.color = Color.red;
					GameObject.Find ("selecionar_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
				...
				GameObject.Find ("selecionar_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
				//Debug.Log ("retornou false");
			}
			
			if (Input.GetKeyDown (KeyCode.L) && selectedObject != null) {
				GameObject.Find ("pegar_red").GetComponent<CanvasRenderer> ().SetAlpha (1.0f);
				OVRPlayerController opc = (OVRPlayerController) GameObject.Find("OVRPlayerController").GetComponent ("OVRPlayerController");
				opc.moedas++;
				GameObject.Find ("Text_moedas").GetComponent<Text> ().text = "Moedas Coletadas: " + opc.moedas;
				GameObject.Destroy (selectedObject);
				//GameObject.Find ("pegar_red").GetComponent<CanvasRenderer> ().SetAlpha (0.0f);
			}
